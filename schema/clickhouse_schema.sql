...
(
    ...
    `vmodem_sysop` String DEFAULT '',
    `vmodem_location` String DEFAULT '',
    `vmodem_addresses` Array(String) DEFAULT [],
    `vmodem_detail` String DEFAULT '',
    `vmodem_call_outcome` String DEFAULT '',
    `vmodem_banner` String DEFAULT '',
    `is_operational` Bool,
...
-- FTS-4010 netmail PING/TRACE measurements (see schema/migrations/016_ping_tests.sql
-- for the full commentary). One row per ping sent, replaced as its state
-- advances; every reply the poller saw kept verbatim. Readers use FINAL.
