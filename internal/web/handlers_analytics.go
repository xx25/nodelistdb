...
func (s *Server) DomainExpirationHandler(w http.ResponseWriter, r *http.Request) {
	...
	normalizeWhoisStatuses(results)
...
func (s *Server) DomainNodesHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Reached from either the domain-expiration report (default) or a
	// registrar drill-down — point the back link at whichever it was.
	backURL := "/analytics/domain-expiration"
	backLabel := "Back to Domain Expiration Report"
	if reg := strings.TrimSpace(r.URL.Query().Get("from_registrar")); reg != "" {
		backURL = "/analytics/registrars?registrar=" + url.QueryEscape(reg)
		backLabel = "Back to " + reg
	}
...
// normalizeWhoisStatuses rewrites raw WHOIS status strings to user-friendly
// display values. Registry-specific codes like "connect" (.de), "ok",
// "clientTransferProhibited" all mean the domain is active; only statuses
// containing inactive keywords are preserved verbatim.
func normalizeWhoisStatuses(results []storage.DomainWhoisResult) {
	for i := range results {
...
// normalizeRegistrarName cleans up a raw WHOIS registrar name for grouping.
// Some registries (notably Nominet for .uk) append a "[Tag = NAME]" suffix to
// the registrar name, which would split one registrar into several groups.
func normalizeRegistrarName(name string) string {
	name = strings.TrimSpace(name)
	if i := strings.Index(name, "[Tag"); i >= 0 && strings.HasSuffix(name, "]") {
		name = strings.TrimSpace(name[:i])
	}
	return name
}

// registrarGroupKey folds spelling variants of the same registrar name into
// one group key. Different registries report the same registrar with
// different casing and trailing punctuation ("Cloudflare, Inc" vs
// "Cloudflare, Inc.", "GANDI SAS" vs "Gandi SAS").
func registrarGroupKey(name string) string {
	return strings.ToLower(strings.TrimRight(name, " .,"))
}

// RegistrarStat aggregates WHOIS registrar usage across node domains
...
// RegistrarsHandler shows which registrars the node domains are registered with.
...
func (s *Server) RegistrarsHandler(w http.ResponseWriter, r *http.Request) {
	...
	selected := strings.TrimSpace(r.URL.Query().Get("registrar"))
	selectedKey := ""
	if selected != "" {
		selectedKey = registrarGroupKey(normalizeRegistrarName(selected))
	}

	type agg struct {
		domains  int
		nodeSet  map[string]struct{} // unique nodes across the group's domains
		variants map[string]int      // spelling variant → domain count
	}
	byRegistrar := make(map[string]*agg)
	var selectedDomains []storage.DomainWhoisResult
	totalDomains := len(results)
	unknownCount := 0

	for _, res := range results {
		name := normalizeRegistrarName(res.Registrar)
		key := registrarGroupKey(name)
		if key == "" {
			unknownCount++
			continue
		}
		a := byRegistrar[key]
		if a == nil {
			a = &agg{nodeSet: make(map[string]struct{}), variants: make(map[string]int)}
			byRegistrar[key] = a
		}
		a.domains++
		for _, nk := range res.NodeKeys {
			a.nodeSet[nk] = struct{}{}
		}
		a.variants[name]++
		if selectedKey != "" && key == selectedKey {
			selectedDomains = append(selectedDomains, res)
		}
	}

	stats := make([]RegistrarStat, 0, len(byRegistrar))
	for _, a := range byRegistrar {
		// Display the group's most common spelling variant
		display := ""
		best := -1
		for v, c := range a.variants {
			if c > best || (c == best && v < display) {
				display, best = v, c
			}
		}
		var percent float64
		if totalDomains > 0 {
			percent = float64(a.domains) * 100 / float64(totalDomains)
		}
		stats = append(stats, RegistrarStat{
			Registrar:   display,
			DomainCount: a.domains,
			NodeCount:   len(a.nodeSet),
			Percent:     percent,
		})
	}
	sort.Slice(stats, func(i, j int) bool {
		if stats[i].DomainCount != stats[j].DomainCount {
			return stats[i].DomainCount > stats[j].DomainCount
		}
		return stats[i].Registrar < stats[j].Registrar
	})
	sort.Slice(selectedDomains, func(i, j int) bool {
		return selectedDomains[i].Domain < selectedDomains[j].Domain
	})

	selectedNodes := make(map[string]struct{})
	for _, d := range selectedDomains {
		for _, nk := range d.NodeKeys {
			selectedNodes[nk] = struct{}{}
		}
	}
	...
	data := struct {
		...
		SelectedDomains   []storage.DomainWhoisResult
